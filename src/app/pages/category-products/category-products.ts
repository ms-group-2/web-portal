...
@Component({
  ...
  imports: [
    Header, Footer, TranslatePipe, RouterLink, ProductCardComponent,
    MatIcon, BottomSheet, MobileFilterBar, SortBottomSheet, SortOptionsList, PriceRangeFilter, SpecFilterList,
  ],
...
export class CategoryProducts implements OnInit {
  ...
  /** True when price "from" is greater than "to" — no products can match. */
  priceRangeInvalid = computed(() => this.minPrice() > this.maxPrice());

  activeFilterCount = computed(() => {
    let count = 0;
    const selected = this.selectedFilters();
    for (const optionIds of Object.values(selected)) {
      if (optionIds && optionIds.length > 0) count++;
    }
    if (this.sortBy()) count++;
    if (this.minPrice() > 0 || this.maxPrice() < this.categoryMaxPrice()) count++;
    return count;
  });
