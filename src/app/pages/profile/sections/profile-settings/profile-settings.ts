...
export class ProfileSettingsComponent implements OnInit {
  ...
  private patchFormWithProfile(profile: any) {
    this.form.patchValue({
      ...
      phone: PhoneUtil.formatForInput(profile.phone_number),
  ...
  onPhoneInput(event: Event) {
    ...
    const sanitized = PhoneUtil.sanitize(input.value);

    if (!sanitized || !sanitized.startsWith('+995')) {
      const digitsOnly = sanitized.replace(/\+/g, '');
      input.value = digitsOnly ? `+995${digitsOnly}` : '+995';
    } else {
  ...
  save() {
    ...
    const normalizedPhone = PhoneUtil.normalizeForApi(formValues.phone ?? '');
